...
const initDb = (callback) => {
    ...
    MongoClient.connect(process.env.MONGODB_URL)
        .then((client) => {
            // Use explicit database name "finalProject"
            // If connection string has a different database name, extract it as fallback
            const urlParts = process.env.MONGODB_URL.split('/');
            const dbNameFromUrl = urlParts.length > 3 ? urlParts[3].split('?')[0] : 'finalProject';
            
            // Explicitly use "finalProject" as database name
